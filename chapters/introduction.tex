...
Ever since the introduction of media on the internet, the amount available to the public has grown significantly. Music streaming options make it possible to listen to millions of songs, and their catalogues are increasing rapidly. This increase forces them to find a way of processing them in a reasonable amount of time if they ever want to offer a decent service. In 2019, Spotify alone offers more than 50 million songs.\footnote{https://investors.spotify.com/} When we assume that each song has a duration of 3 minutes, this roughly equals to over 285 years of music.
...
There are already a multitude of representations available for music, either with features engineered by hand using domain knowledge or learned using machine learning techniques.

One of the hand crafted representations is audio fingerprinting and as the name suggests, the representation can be seen as a `fingerprint', or a minimal summary, of a song. This is perfect for identification and quick lookup of a particular song, but it ends up throwing away a lot of other information, meaning that its application is narrowed down significantly. We can determine if two songs are the same by comparing the fingerprints, but simultaneously lost the ability to determine the musical properties (tempo, key, energy, etc.) of the song.

An example of a learned representation is one suggested by Dieleman et al. \cite{Dieleman2014}, where they compare learning features using raw audio as direct input (end-to-end learning) to using spectrograms. By using a Convolutional Neural Network (CNN), they try and predict the tags associated with the song. Though promising, they couldn't match the performance of the spectrogram-based approach yet. Even though end-to-end learning is still incredibly hard to achieve a high level of performance with, the biggest advantage is that we don't lose the musical properties mentioned previously.

These properties can be grouped under the umbrella term called \textbf{musical attributes} and we want to find a balance in a compact representation, while also keeping as much information about these musical attributes as possible. Specializing too much in a specific task and throwing away musical attributes may be the most compact representation, but applying it to a different task proves to be difficult. On the other hand, end-to-end learning retains almost all the information on the musical attributes, but requires the (samples of) the songs in its raw form. 

\textbf{Compactness} could be interpreted traditionally, where redundancy is removed to achieve a smaller size and thus resulting in a lower dimension. But another form of compactness is sparsity, where the dimensionality could even increase, but most of the entries are zero and only a few dimensions are used at most, resulting in a more efficient method of working with the data which speeds up the computation significantly \cite{Fyfe1995}.

To find a middle ground in this, we propose the following research question:

\begin{itemize}
	\item{How can we obtain a \textbf{compact} \textbf{representation} of music without losing information about its \textbf{musical attributes}?}
\end{itemize}


As for the scientific contribution by answering this question, it offers an alternative representation for music where speed and compactness is key, with respect to the musical attributes of the songs. While doing so, it also provides a middle ground to either throwing away all musical attributes and working with raw music as direct input, allowing it to be used in more general settings.
